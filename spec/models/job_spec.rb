...
describe Job do
	...
	it "is valid with a title, job_type, external_link, job_type, address_id, company_id, posted_at, expires_at" do
	...
	it "is invalid without an address_id" do
		expect(build(:job, address_id: nil)).to have(1).errors_on(:address_id)
	end

	it "is invalid without a company_id" do
		expect(build(:job, company_id: nil)).to have(1).errors_on(:company_id)
	end
